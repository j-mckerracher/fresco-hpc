...
# --- Configuration ---
ROOT_DIR = Path("<LOCAL_PATH_PLACEHOLDER>/projects/stampede-step-3/input")
OUTPUT_DIR = Path("<LOCAL_PATH_PLACEHOLDER>/projects/stampede-step-3/output")
...
log_file = "logs/stampede-final-cleanup.log"
...
def transform_dataframe(df: pl.DataFrame) -> tuple[pl.DataFrame, bool]:
    ...
    # Columns that should get "_S" suffix (based on goal format analysis)
    append_s_cols = ["account", "queue", "host", "jid", "jobname", "host_list", "username"]
    ...
    # 2. Transform columns that need "_S" suffix
    for col_name in append_s_cols:
        if col_name in df.columns:
            ...
            if df[col_name].dtype != pl.Utf8:
                if col_name == "jid":
                    # Special handling for jid column - prepend "JOB" to numeric values
                    transformations.append(
                        ("JOB" + pl.col(col_name).cast(pl.Utf8) + "_S")
                        .alias(col_name)
                    )
                elif col_name == "host_list":
                    # Optimized handling for host_list - use chunked processing for better performance
                    # Remove the length check and always apply transformation
                    logging.debug("Applying host_list transformation (may take longer for large strings)")
                    transformations.append(
                        pl.col(col_name).cast(pl.Utf8)
                        .str.replace_all(r"([A-Z]+\d+)", r"${1}_S")
                        .alias(col_name)
                    )
                else:
                    transformations.append(
                        (pl.col(col_name).cast(pl.Utf8)
                        + "_S")  # Fixed: use simple string concatenation
                        .alias(col_name)
                    )
            else:
                if col_name == "jid":
                    # Special handling for jid column - prepend "JOB" to values
                    transformations.append(
                        ("JOB" + pl.col(col_name) + "_S")
                        .alias(col_name)
                    )
                elif col_name == "host_list":
                    # Optimized handling for host_list - use chunked processing for better performance
            ...
            logging.debug(f"Added '{col_name}' transformation with _S suffix.")

    # 3. Keep all other columns as-is
    other_cols = [col for col in df.columns if col not in append_s_cols]
